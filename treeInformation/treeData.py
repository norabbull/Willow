...
class treeInfo:
    ...
    def __init__(self):
        ...
        group_types = list of sets with population names.Index 0 = super, index 1 = sub 
    ...
    def setDistMat(self, path):
        self.dist_mat = pd.read_csv(path, index_col = 0)
        
    def setGeneName(self, path):
        """
        Input: 
            dist_mat_path: path to cd-file. 
        Function: 
            Filters out treename (usually a gene name). 
        Return: 
            treename as string
        """
        subName = re.sub('^.:.*/ENS', 'ENS', self.path)
        self.gene_name = re.sub('___CopD.csv$','', subName)
    
    def setGroupInfo(self, path):
        """
        path: String path to group type info file
                group types = super and sub
        list of sets of included classification names in 
                    super- and sub pops respectivly
        """
        group_info = pd.read_csv(path, delimiter='\t')
        ...
        self.group_info = [super_pops, sub_pops]
    ...
    def calcNonZerosForGroups(self, 
                             dist_mat_path,
                             group_types, 
        ...
        Input: 
            ...
            group_types: Defines populations in classificationType. 
        Function:
            COunt number of 
        ...
        TO DO: 
            create group_types and groups with functions within this function.
            Have to fix imports and project structure first. 
        """
        dist_mat = load_dist_mat(dist_mat_path)   
        sample_info = getSampleInfo()
        ...
        if classificationType=='super':
            sup_sums = {pop : 0 for pop in group_types[0]}   # Placeholder for sum values
            sup_sample_info = dict([val[0], val[1]] for val in sample_info.values())
            
        elif classificationType=='sub':
            sub_sums = {pop : 0 for pop in group_types[1]}   # Placeholder for sum values
        ...
        #     sums1 = {pop : 0 for pop in group_types[0]}   # Placeholder for sum values
        #     sums2 = {pop : 0 for pop in group_types[1]}
...
# =============================================================================
    ...
    dist_mat_path = 'E:/Master/cophenetic_dists/ENSG00000000938___FGR___CopD.csv'
    # pop_
    group_type_sets = setGroupInfo('C:/Users/norab/MasterDisaster/Data/real_tree_data/phydist_population_classes.tsv')
    
    # test calcNonZerosForGroups
    test = calcNonZerosForGroups(dist_mat_path, 
                                group_types=group_type_sets, 
                                classificationType='all',
                                percent = True)
